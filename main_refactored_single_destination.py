...
def open_routing_debug_map_p2p(
    ...
    *,
    ...
    修改版：只顯示 Collision, S_edges, A* Raw, Final 四種圖層。
    """
    ...
    if len(pair.start_pick) > 0 and len(pair.end_pick) > 0:
        ...
        try:
            ...
            print(f"[route] OK | nodes={len(path)}")
        ...
        if path and len(path) >= 2:
            ...
            if collision is None:
                ...
                collision_used = collision # 保存下來給繪圖用
    ...
    # --- 移除 Nodes Layer (根據需求移除) ---
    # add_points(C_nodes, "C_nodes", "blue", radius=1, sample=c_sample)
    # add_points(S_nodes, "S_nodes", "green", radius=3, sample=s_sample)
    # add_points(dfGB, "Gate_B", "red", radius=5)
    ...
    if path and len(path) >= 2:
        ...
        # Path: Repaired - 移除
        # if path_ll_for_simplify:
        #     fgR = folium.FeatureGroup(name="Path: Repaired", show=True)
        #     folium.PolyLine([[p[1], p[0]] for p in route_polyline_dateline_safe(path_ll_for_simplify)], color="green", weight=4).add_to(fgR)
        #     fgR.add_to(m)
...
VNNGH: 105.8149, 19.3117'''
        ...
        # stretch=1 表示它會跟下面的 Log Box 平分剩餘空間
        ...
        # stretch=1 表示它會跟上面的 Port Box 平分剩餘空間
        ...
        self.web_view.setHtml("<html><body style='display:flex;justify-content:center;align-items:center;font-family:sans-serif;color:#555;'><h1>Ready. Enter coordinates and click Calculate.</h1></body></html>")
    ...
    def run_routing(self):
        o_ll, d_ll = self.parse_latlon(self.input_origin.text()), self.parse_latlon(self.input_dest.text())
        if not o_ll or not d_ll:
            self.log("Error: Invalid coords.")
            return

        self.btn_calc.setEnabled(False)
        self.log(f"Computing: {o_ll} -> {d_ll}")
        try:
            html = open_routing_debug_map_p2p(
                self.out, o_ll, d_ll, html_path="aoi_p2p_map_ui.html",
                do_repair=True, do_simplify=True
            )

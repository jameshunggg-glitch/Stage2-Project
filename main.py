        ...
        if path is None or len(path) < 2:
            ...
            if collision is None:
                ...
                if do_simplify and path_ll_for_simplify is not None and len(path_ll_for_simplify) >= 2:
                    ...
                    # Decide final route polyline (priority: simplified > repaired_full > original A*)
                    final_ll = None
                    if path_simplified is not None and len(path_simplified) >= 2:
                        final_ll = path_simplified
                    elif path_ll_for_simplify is not None and len(path_ll_for_simplify) >= 2:
                        final_ll = path_ll_for_simplify
                    elif path is not None and len(path) >= 2:
                        final_ll = [(float(p[0]), float(p[1])) for p in path]
                    ...
                    if final_ll is not None and len(final_ll) >= 2:
                        ...
                        # optional: write to out so you can inspect later
                        try:
...
# === call
# =========================
origin_ll = (127.09912, 13.3102)
dest_ll   = (17.47887, 42.36624)
